...
// SwapToken contract: Facilitates token swapping between PureDollarToken and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between PureDollarToken and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow PureDollarToken and FlowToken vaults from the signer's storage
        let PureDollarTokenVault = signer.borrow<&PureDollarToken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow PureDollarToken Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow the Minter capability from PureDollarToken
        let minterRef = signer.getCapability<&PureDollarToken.Minter>(/public/Minter).borrow()
            ?? panic("Could not borrow a reference to PureDollarToken Minter")
        ...
        // Mint new PureDollarTokens and deposit them into the vault
        let newPureDollarTokenVault <- minterRef.mintToken(amount: mintedAmount)
        PureDollarTokenVault.deposit(from: <-newPureDollarTokenVault)
