...
//PureDollarToken contract: Implements a fungible token on the Flow blockchain
pub contract PureDollarToken: FungibleToken {

    // Total supply of PureDollarToken
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @PureDollarToken.Vault
        ...
        // Destroy the vault
        destroy() {
           PureDollarToken.totalSupply = PureDollarToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
           PureDollarToken.totalSupply =PureDollarToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&PureDollarToken.Minter>(/public/Minter, target: /storage/MinterStorage)
